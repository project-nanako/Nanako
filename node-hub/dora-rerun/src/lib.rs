...
use rerun::{
    ...
    external::{re_sdk_comms, re_types::ArrowBuffer},
    ImageFormat, Text,
};
pub mod series;
pub mod urdf;
...
pub fn lib_main() -> Result<()> {
    ...
    let server_addr = std::env::var("RERUN_SERVER").unwrap_or("127.0.0.1:9876".to_string());

    let rec = rerun::RecordingStreamBuilder::new("dora-rerun")
        .connect_tcp_opts(
            server_addr.parse().expect("Invalid address"),
            re_sdk_comms::default_flush_timeout(),
        )
